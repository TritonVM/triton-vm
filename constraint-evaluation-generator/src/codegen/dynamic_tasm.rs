...
const OUT_ARRAY_OFFSET: usize = {
    let mem_page_size = DynamicTasmConstraintEvaluationMemoryLayout::MEM_PAGE_SIZE;
...
mod tests {
    ...
    fn print_constraints_as_tasm(constraints: &Constraints) {
        let tasm = DynamicTasmBackend::constraint_evaluation_code(constraints);
