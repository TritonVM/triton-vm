...
2. A multi-table Permutation Argument shows that all clock jump differences greater than one, from all memory-like tables (i.e., including the [OpStack Table](operational-stack-table) and the [JumpStack Table](jump-stack-table)), are contained in the `cjd` column of the [Processor Table](processor-table). The running product for this argument is contained in the `rpcjd` column.