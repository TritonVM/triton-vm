...
$$ S : \mathbb{F}_p \rightarrow \mathbb{F}_p,  x \mapsto \rho \circ \mu \circ L^8 \circ \mu \circ \sigma  (x) $$

The components are:

 - $\sigma : \mathbb{F}_p \rightarrow \mathbb{F}_p^8, x \mapsto (a, b, c, d, e, f, g, h)$ where all four outputs are at most 8 bits wide and $x = a + 2^{8} \cdot b + 2^{16} \cdot c + 2^{24} \cdot d + 2^{32} \cdot e + 2^{40} \cdot f + 2^{48} \cdot g + 2^{56} \cdot h$.
 - $\mu : \mathbb{F}_p^8 \rightarrow \mathbb{F}_p^8, (a, b, c, d, d, e, f, g, h) \mapsto (a, b, c, d, 2^{16}-1 - e, 2^{16}-1 - f, 2^{16}-1 - g, 2^{16}-1 - h)$.
 - $L : \mathbb{F}_ p \rightarrow \mathbb{F}_ p$ is defined only for field elements that are at most 8 bits wide. Identifying this subset of $\mathbb{F}_ p$ with $\mathbb{F}_ {2^{8}+1}$, the lookup table $L$ computes $L : \mathbb{F}_ {2^{8}+1} \rightarrow \mathbb{F}_ {2^{8}+1}, x \mapsto x^3$.
...
These benchmarks were obtained on a Intel(R) Core(TM) i7-10750H CPU @ 2.60GHz. On this machine Tip5 is 11.6× faster than Rescue-Prime Optimized.
