...
The cascade construction arithmetizes a composite lookup gate in terms multiple lookups into component gates followed by combining the looked-up outputs. For example, supose that a 16-bit wide map can be represented as the concatenation of two 8-bit wide lookups. Then this 16-bit wide map can be arithmetized with one cascade table and one 8-bit wide lookup table. This cascade table is the server providing 16-bit wide input-output pairs to the external client. Simultaneously, it is the client of the 8-bit wide input-output pair lookup.
