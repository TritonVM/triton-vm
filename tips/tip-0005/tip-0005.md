...
To establish the correct subset relation, we need to add two columns in total. There is one column `mul` that represents the number of times the pair in question is looked up. In terms of extension columns, there is one running sum column, `sum`.

### Intuition

Using random weights from the verifier `a, b`, the input and output columns are compressed into one random linear combination. It then suffices to show that the set of random linear combinations used by the Hash Table, is a subset of the random linear combinations in the lookup table. For the purpose of presenting the lookup argument, the index `[i]` is dropped; this omission would suggest incorrectly that the Hash Table contains only one input-output pair, and the written explanation matches with this suggestion.
...
On the side of the Lookup Table, two columns instead of three are needed to probe this identity in the random point $\alpha$.

 - the running product `rph` and multiplicity-weighted formal derivative `mwfdh` can be dropped;
 - base column `mul` still contains the multiplicity with which the given row is queried;
 - extension column `sum` contains the running sum of `mul / (α - rlc)`.

However, the main benefit is that on the Hash Table only *one* extension column is needed instead of two. Specifically, the running product `rph` and formal derivative `fdh` can be dropped in favor of a single column, the logarithmic derivative `ldh`.

To update `ldh`, recall that the running product column `rph` was defined to accumulate one *factor* in every row. Since `ldh` is defined to contain the logarithmic derivative of `rph` in every row, we can use the eponymous property to populate it. Speficially, the running product update rule `rph* = rph * (α - rlc*)` becomes `ldh* = ldh + 1/(α - rlc*)`, where the asterisk `*` indicates the respective element from the next row.
