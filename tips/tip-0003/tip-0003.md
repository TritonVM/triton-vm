...
Therefore, valid differences belong to $V = \lbrace 1, \dots, T-1 \rbrace \subseteq \mathbb{F}_p$ and invalid ones to $\mathbb{F}_p\setminus V$.
As long as $T \ll p/2$, there is no overlap between these sets.
As a result, in this regime, showing that a clock jump difference is also some clock cycle guarantees that the jump is forward-directed.
...
## Intuition
 ...
 It comes with the matching instruction `set_ramp`, which copies `st0` into `ramp`, and counterpart instruction `get_ramp`, its reciprocal.
...
The total is 7 extra base columns, 6 extension columns, and 2 instructions.
...
The column $\mathsf{clk\_di}$ containts the inverse-or-zero of the two consecutive clocks, minus one.
