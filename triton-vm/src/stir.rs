...
/// The full transcript of a [STIR verification](Stir::verify).
///
/// Includes the data used or generated by the verifier:
/// - [proof items](ProofItem) received from the [prover](Stir::prove)
...
impl Stir {
    ...
    pub fn verify(
        ...
        let round_params = self.round_params()?;
        let initial_polynomial_commitment = proof_stream.dequeue()?.try_into_merkle_root()?;

        let mut stir_to_stark_links = None;
        let mut previous_quotienting_data = None;
        let mut domain = self.initial_domain()?;
        let mut commitment_to_previous_polynomial = initial_polynomial_commitment;
        for num_queries in round_params.round_queries {
            let folding_randomness = proof_stream.sample_scalars(1)[0];
