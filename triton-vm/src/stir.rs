...
pub struct Transcript {
    ...
    /// One round-transcript per full round of STIR.
    pub rounds: Vec<RoundTranscript>,

    /// The transcript of the final round of STIR.
    pub final_round: FinalRoundTranscript,
}

/// A transcript of a single full round of a [STIR verification](Stir::verify).
///
/// See [`Transcript`] for a full explanation.
#[non_exhaustive]
#[derive(Debug, Clone, Eq, PartialEq)]
...
/// A transcript of the final round of a [STIR verification](Stir::verify).
///
/// See [`Transcript`] for a full explanation.
#[non_exhaustive]
#[derive(Debug, Clone, Eq, PartialEq)]
...
/// The parameters derived for a [STIR](Stir) instance.
#[derive(Debug, Clone, Eq, PartialEq)]
struct RoundParams {
...
impl Stir {
    ...
    pub fn prove(
        ...
        let folding_factor = self.round_params.folding_factor;
        ...
        for num_queries in &self.round_params.round_queries {
        ...
        let queried_indices = proof_stream
            .sample_indices(domain.len(), self.round_params.final_num_in_domain_queries);
    ...
    pub fn verify(&self, proof_stream: &mut ProofStream) -> VerifierResult<Transcript> {
        ...
        let mut round_transcripts = Vec::with_capacity(self.round_params.round_queries.len());
        let mut domain = self.initial_domain;
        ...
        for num_queries in &self.round_params.round_queries {
            ...
            round_transcripts.push(RoundTranscript {
                domain,
                folding_factor: self.round_params.folding_factor,
                folding_randomness,
                queried_indices,
    ...
    fn extract_inclusion_proof(
        ...
        let folded_domain = round_domain.pow(self.round_params.folding_factor).unwrap();
    ...
    fn partial_codeword(
        ...
        let folded_domain_len = domain.pow(self.round_params.folding_factor).unwrap().len();
...
impl RoundTranscript {
    ...
    pub fn folded_queried_indices(&self) -> impl Iterator<Item = usize> {
        Transcript::folded_queried_indices(self.domain, self.folding_factor, &self.queried_indices)
    }
}
...
mod tests {
    ...
    fn prove_and_verify_low_degree_polynomial(
        ...
        prop_assert_eq!(&prover_indices, transcript.first_round_indices());

        for (idx, value) in prover_indices
            .into_iter()
            .zip_eq(transcript.partial_first_codeword)
        {
