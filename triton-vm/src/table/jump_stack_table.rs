...
impl JumpStackTable {
    ...
    pub fn fill_trace(
        ...
        for row_idx in 0..aet.processor_matrix.len() - 1 {
            ...
            if curr_row[usize::from(JSP)] == next_row[usize::from(JSP)] && clk_diff.value() > 1 {
    ...
    pub fn pad_trace(
        ...
        let rows_that_need_moving = jump_stack_table
            .slice(s![
        ...
        rows_that_need_moving.move_into(
            &mut jump_stack_table.slice_mut(s![rows_that_need_moving_insertion_idx.., ..]),
        );
