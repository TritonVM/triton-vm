...
impl MasterBaseTable {
    ...
    pub fn extend(&self, challenges: &Challenges) -> MasterExtTable {
        ...
        let mut randomized_trace_extension_table =
            Array2::<XFieldElement>::zeros((num_rows, NUM_EXT_COLUMNS).f());

        // Vec::with_capacity(num_table_elements);
        // #[allow(clippy::uninit_vec)]
        // unsafe {
        //     // Use of unsafe code because it is faster than the alternative, which is
        //     // `vec![XFieldElement::zero(); num_table_elements]` and which is slower
